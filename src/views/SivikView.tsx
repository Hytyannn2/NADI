        ...
        <div className="p-5 h-full flex flex-col relative z-0" style={{ background: 'var(--bg-base)' }}>
            ...
            >
                ...
                <p className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>
                    Civic wallet & reputation · Kelantan
                </p>
            </motion.div>
            ...
            >
                <div className="relative z-10 flex flex-col items-center text-center">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs text-white/70 font-medium">Civic Points</span>
                    </div>
                    {isLoading ? (
                    ...
                    <p className="text-xs text-white/50 font-medium">
                        Earn points through civic reports, volunteering & community participation
                    </p>
                </div>
            </motion.div>
            ...
            >
                ...
                <div className="rounded-2xl p-4 text-center" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)' }}>
                    ...
                    <p className="text-[8px] font-bold mt-0.5" style={{ color: 'var(--success)' }}>{trustLabel}</p>
                </div>
                ...
                <div className="rounded-2xl p-4 text-center" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)' }}>
                    ...
                    <p className="text-[10px] font-medium" style={{ color: 'var(--text-muted)' }}>Quests</p>
                </div>
            ...
            >
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-semibold" style={{ color: 'var(--text-muted)' }}>Civic Reputation Score</span>
                    </div>
            ...
            >
                {(['ledger', 'leaderboard'] as const).map(s => (
                    ...
                    >{s === 'ledger' ? 'Activity Ledger' : 'Leaderboard'}</button>
                ))}
            </motion.div>
            ...
            {activeSection === 'leaderboard' && (
                <div className="space-y-2">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: 'var(--accent)' }}>Community Leaderboard</h3>
                    {leaderboard.map((u, i) => (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[8px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{u.mukim}</p>
                                </div>
                            </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <h3 className="text-[10px] font-bold uppercase tracking-widest mb-6 flex items-center justify-between" style={{ color: 'var(--accent)' }}>
                ...
                {isLoading ? (
                                ...
                                <div key={tx.id} className={`flex items-center justify-between p-4 ${i !== Math.min(wallet.transactions.length, 10) - 1 ? 'border-b' : ''}`} style={{ borderColor: 'var(--border-default)' }}>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>{tx.notes || tx.category}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-[8px] font-bold uppercase tracking-widest mt-1" style={{ color: 'var(--text-muted)' }}>{tx.time}</p>
                                    </div>
                                </div>
                            );
                        })}
                    ...
                    <div className="text-center py-10 rounded-3xl text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)', border: '1px dashed var(--border-default)' }}>
