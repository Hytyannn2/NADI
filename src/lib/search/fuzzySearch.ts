...
/**
 * Creates a pre-configured Fuse.js search instance for structured collections.
 */
export function createCivicFuse<T extends Record<string, any>>(
    list: T[], 
    keys: (keyof T | string)[]
): Fuse<T> {
...
/**
 * Detects search intent for evacuation shelters (proximity, district, capacity, or facilities).
 */
export function detectPPSSearchIntent(query: string | null | undefined): {
    type: 'nearest' | 'district' | 'name' | 'capacity' | 'facility';
    district?: string;
} {
    if (!query || !query.trim()) return { type: 'name' };

    const normalized = normalizeCivicSearch(query);

    if (normalized.includes('terdekat') || normalized.includes('nearest') || normalized.includes('dekat')) {
        return { type: 'nearest' };
    }

    const districts = [
        'kota bharu', 'pasir mas', 'tumpat', 'bachok', 'machang',
        'tanah merah', 'gua musang', 'kuala krai', 'jeli', 'pasir puteh'
    ];
    for (const d of districts) {
        if (normalized === d || normalized.includes(d)) {
            return { type: 'district', district: d };
        }
    }

    if (normalized.includes('kapasiti') || normalized.includes('muat')) {
        return { type: 'capacity' };
    }

    if (normalized.includes('oku') || normalized.includes('dapur') || normalized.includes('tandas')) {
