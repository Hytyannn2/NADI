...
export default function BottomNav({ tabs, activeTab, onTabSwitch }: BottomNavProps) {
  return (
    <nav className="absolute bottom-0 left-0 right-0 border-t bg-[var(--bg-card)] border-[var(--border-default)]" style={{ paddingBottom: 'env(safe-area-inset-bottom, 8px)' }}>
      <div className="flex justify-around items-center px-2 pt-2 pb-2">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <div className={`p-2 rounded-xl mb-0.5 transition-all duration-200 ${
                isActive 
                  ? 'bg-[var(--accent)]/10' 
                  : ''
              }`}>
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute -top-0.5 w-8 h-0.5 rounded-full bg-[var(--accent)]"
